...
def buscar_trabajador_por_id(id_trabajador):
    try:
        from database.conexion import conectar
        conexion = conectar()
        if conexion is None:
        ...
        cursor = conexion.cursor(dictionary=True)
        cursor.execute(
            "SELECT id_trabajador, nombre, ap_paterno, ap_materno, cedula_profesional, puesto FROM trabajadores WHERE id_trabajador = %s",
            (id_trabajador,)
        )
        ...
        print(f"Error al buscar trabajador por ID: {e}")
        return None
